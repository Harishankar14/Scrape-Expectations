...
function mapItem(item, index) {
  ...
  return {
    ...
    reviews:     item.reviews || item.reviews_cnt || item.reviews_count || 0,
    delivery:    item.delivery || item.shipping || null,
    tags:        item.extensions || item.badges || item.tags || [],
    trustLevel:  Math.floor(Math.random() * 21) + 80,
...
app.get('/api/search', async (req, res) => {
  ...
  const token = process.env.BRIGHT_DATA_API_TOKEN || process.env.BRIGHT_DATA_API_KEY;
  if (!token || token === 'your_api_token_here') {
    console.log('[Scrape-Expectations] No API token — using mock data.');
    return setTimeout(() => res.json({ success: true, query, results: mockResults }), 300);
  }

  const started = Date.now();
  let rawItems = [];
  try {
    rawItems = await scrapeShopping(query, token);
  } catch (e) {
